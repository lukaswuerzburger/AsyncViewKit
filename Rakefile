...
namespace :test do
  ...
  task ios: :prepare do
    run('test', 'AsyncViewKit', 'Debug', 'iPhone 7', '10.3.1')
    run('test', 'AsyncViewKit', 'Debug', 'iPhone 8', '11.4')
    run('test', 'AsyncViewKit', 'Debug', 'iPhone Xs', '12.4')
    run('test', 'AsyncViewKit', 'Debug', 'iPhone SE (2nd generation)', '13.5')
    run('test', 'AsyncViewKit', 'Debug', 'iPhone SE (2nd generation)', '14.5')
    run('test', 'AsyncViewKit', 'Debug', 'iPhone SE (2nd generation)', '15.2')
  end

  desc 'Build the Demo App'
  task ios_example: :prepare do
    run('build', 'AsyncViewKitDemo', 'Release', 'iPhone SE (2nd generation)', '13.5')
  end
end
...
def run(operation, scheme, configuration, device, os)
    sh("xcodebuild -workspace AsyncViewKit.xcworkspace -scheme '#{scheme}' -sdk 'iphonesimulator' -destination 'platform=iOS Simulator,name=#{device},OS=#{os}' -configuration #{configuration} clean #{operation} | xcpretty") rescue nil
    if $?.success?
