...
has_pod_changes = !git.modified_files.grep(/^AsyncViewController\//).empty?
...
* **feature/improvement/bugfix** #{github.pr_title}\s\s
  [#issue_number](https://github.com/lukaswuerzburger/AsyncViewController/issues/issue_number) by @#{github.pr_author}
```
  MARKDOWN
